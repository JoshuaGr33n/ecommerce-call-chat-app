...
from .models import IT_ChatMessage, IT_CallRecording
...
class Consumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        if eventType == 'login':
            ...
            chat = IT_ChatMessage.objects.filter(
    ...
    def save_chat_message(self, sender_username, receiver_username, message):
        ...
        #    flag = chat.first().flag
        if(sender.role == 3):
            user_chat = IT_ChatMessage.objects.filter(flag=f'flag_{sender_username}')
            if user_chat.count() == 0:
                flag = f'flag_{sender_username}'
                # IT_ChatMessage.objects.create(sender=sender, receiver=receiver, message=message, flag=flag)
                admin_flag = 'Flag'
            else:
                flag = user_chat.first().flag
                # user_chat = IT_ChatMessage.objects.filter(sender=sender)
            ...
            # IT_ChatMessage.objects.create(sender=sender, receiver=receiver, message=message, flag=flag, admin_flag=sender.username)
        ...
        IT_ChatMessage.objects.create(sender=sender, receiver=receiver, message=message, flag=flag, admin_flag=admin_flag)    
    ...
    def handle_audio_message(self, message):
        ...
        recording = IT_CallRecording.objects.filter(
        ...
        # Create and save the IT_CallRecording model instance
        call_recording = IT_CallRecording(
            caller=caller,
            receiver=receiver,
    ...
    def get_unread_messages_count(self, username):
        ...
        return chat_is_read_status(user, IT_ChatMessage)
